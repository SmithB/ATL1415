...
def add_files_to_queue(run_name=None, task_list_file=None, task_glob=None, shell=None, env=None, R_range=None, max_jobs=None, line_count=None):
...
def __main__():
    ...
    for ii in range(len(R_vals)-1):
        ...
        while N_added > 0:
            ...
            N_added = add_files_to_queue(run_name=args.run_name, task_list_file=args.queue_file, task_glob=args.task_glob, shell=args.shell, env=args.environment, R_range=R_range, max_jobs=4000, line_count=line_count)
            if N_added <1:
                continue
