...
#        DEM_file = '/Volumes/ice3/suzanne/REMA_100m_dem.tif'
    ...
    # figure out what the base unit of the time interval is:
    if np.abs(args.delta_t-1/12)<0.001:
        delta_t_str = '1mo'
        src_group='dhdt_001mo'
        period_name='monthly'
    elif np.abs(args.delta_t-1/4) < 0.001:
        delta_t_str = '3mo'
        src_group='dhdt_003mo'
    ...
    avgs = ['1km', '2.5km', '10km','20km','40km']
    for ii, ave in enumerate(avgs):
        filein = os.path.join(args.base_dir , '_'.join(['ATL15', args.region , args.cycles, delta_t_str,  ave, args.Release, args.version]) + '.nc')
        if not os.path.isfile(filein):
            print(f"{filein} does not exist, skipping")
            continue
        print('Making browse figures from ',filein)
        brw_base = filein.replace('.nc','_BRW')

        ds = Dataset(filein)

        x = ds.groups[src_group]['x']
        y = ds.groups[src_group]['y']
        extent=[np.min(x),np.max(x),np.min(y),np.max(y)]

        dhdt = ds.groups[src_group]['dhdt']
        dhdt[:][dhdt[:]==ds.groups[src_group]['dhdt']._FillValue] = np.nan
        ...
        h = ax.imshow(dhdtmn, extent=extent, cmap='Spectral', vmin=max(abs(h05mn),abs(h95mn))*-1.0, vmax=max(abs(h05mn),abs(h95mn)), origin='lower', interpolation='nearest')
        plt.colorbar(h,ax=ax,label='dh/dt, m',shrink=1/2, extend='both')
        ...
        if args.Hemisphere==1:
            plt.figtext(0.1,0.01,f'Figure 1. Average {period_name} rate of height change ({src_group}) at {ave}-resolution, in meters/yr, from cycle {args.cycles[0:2]} to cycle {args.cycles[2:4]}. Map is plotted in a polar-stereographic projection with a central longitude of 45W and a standard latitude of 70N.',wrap=True)
        elif args.Hemisphere==-1:
            plt.figtext(0.1,0.01,f'Figure 1. Average {period_name} rate of height change ({src_group}) at {ave}-resolution, in meters/yr, from cycle {args.cycles[0:2]} to cycle {args.cycles[2:4]}. Map is plotted in a polar-stereographic projection with a central longitude of 0W and a standard latitude of 71S.',wrap=True)
        plt.subplots_adjust(bottom=0.2)
        plt.tight_layout(rect=[0, 0.05, 1, 1])
        fig.savefig(f'{brw_base}_default1.png', bbox_inches='tight')
        ...
        plt.colorbar(h,ax=ax,label='dh/dt standard deviation, m',shrink=1/2, extend='both')
        gl = ax.gridlines(draw_labels=True, dms=True, x_inline=False, y_inline=False, alpha=0.5, linestyle='--', crs=ccrs.PlateCarree())
        gl.top_labels = False
        gl.right_labels = False
        ax.set_title(f'Standard deviation of {period_name} dh/dt: {os.path.basename(filein)}',wrap=True)
        if args.Hemisphere==1:
            plt.figtext(0.1,0.01,f'Figure 2. Standard deviation of {period_name} rate of height change ({src_group}) at {ave}-resolution, in meters, from cycle {args.cycles[0:2]} to cycle {args.cycles[2:4]}. Map is plotted in a polar-stereographic projection with a central longitude of 45W and a standard latitude of 70N.',wrap=True)
        elif args.Hemisphere==-1:
            plt.figtext(0.1,0.01,f'Figure 2. Standard deviation of {period_name} rate of height change ({src_group}) at {ave}-resolution, in meters, from cycle {args.cycles[0:2]} to cycle {args.cycles[2:4]}. Map is plotted in a polar-stereographic projection with a central longitude of 0W and a standard latitude of 71S.',wrap=True)
        plt.subplots_adjust(bottom=0.2)
        plt.tight_layout(rect=[0, 0.05, 1, 1])
        fig.savefig(f'{brw_base}_default2.png', bbox_inches='tight')
    ...
    #    print(glob.glob(f'{args.base_dir.rstrip("/")}/ATL15_{args.region}_{args.cycles}{ave}_{args.Release}_{args.version}_BRW_default*.png'))
        ...
        with h5py.File(brwfile,'r+') as hf:
            ...
            for ii, name in enumerate(sorted(glob.glob(f'{brw_base}_default*.png'))):
...
def main():
    ...
    # process args.grid_spacing from a comma-separated list possibly including fractions
    # to a list of floats
